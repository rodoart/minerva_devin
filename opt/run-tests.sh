...
# LOG MINERVA_NAMES
LOG_ERR_LINUX="${LOGS_DIR_LINUX}/minerva_tests-err_${CURRENT_DATE}.log"
LOG_STD_LINUX="${LOGS_DIR_LINUX}/minerva_tests-std_${CURRENT_DATE}.log"
...
nohup python -m pytest -s --basetemp="${MINERVA_TMP_DIR_LINUX}" tests/ -v -q > "${LOG_ERR_LINUX}" 2> "${LOG_STD_LINUX}" &
